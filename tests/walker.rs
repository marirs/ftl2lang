use ftl2lang::ftl::walk::{walk_source, TranslatableSpan};
...
fn simple_file_extracts_each_message_value() {
    ...
    let spans = walk_source(&src).unwrap();
...
fn placeables_are_excluded_from_spans() {
    ...
    // "Hello, " + "! You have new mail." + "Welcome to " + "."
...
fn selector_variants_each_contribute_spans() {
    ...
    assert!(texts.contains(&"one new message"));
    assert!(texts.contains(&"new messages"));
}
